...
bool RF24Gateway::begin(bool configTUN, bool meshEnable, uint16_t address, uint8_t mesh_nodeID, rf24_datarate_e data_rate, uint8_t _channel) {
  ...
  // FIXME
	...
	channel = _channel;	// NOTE: Currently, the default channel is 97
  ...
  /* Enable the RF24Network multicast relay by default */
...
int RF24Gateway::configDevice(uint16_t address) {
  /* NOTE: default name of the tun/tap device is "tun_nrf24" */
...
int RF24Gateway::allocateTunDevice(char *dev, int flags, uint16_t address) {
  ...
  if((fd = open("/dev/net/tun", O_RDWR)) < 0) {
    /* return the file descriptor of the tun device */
    return fd;
  }
  ...
  /* set our tun interface to persistent */
...
void RF24Gateway::poll(uint32_t waitDelay) {
  handleRX(waitDelay);
  rfNoInterrupts();   // disable interrupts for a moment...
  /*
   * The gateway.poll() function is called manually when interrupts are enabled,
   * so we'll check for things such as a full radio RX buffer, or interrupts
   * that we've missed, and handle them here.
   */
  if(radio.rxFifoFull()) {
    fifoCleared = true;
  ...
  handleRadioOut();
  rfInterrupts();     // ...and re-enable interrupts
}
...
void RF24Gateway::handleRadioOut() {
  ...
  while(!txQueue.empty() && network.external_queue.size() == 0) {
    ...
    if(!config_TUN ) {
      /*
       * NOTE: while TAP mode can use RF24Mesh for address assignment, address
       * resolution will still be handled by the ARP protocol.
       */
			uint32_t RF24_STR = 0x34324652; // Identifies the MAC addr as an RF24 MAC
      ...
      if(macData.rf24_Verification == RF24_STR) {
        ...
        if(msgTx->size <= 42) {	
          if(thisNodeAddress == 00) { // Master Node
            uint32_t arp_timeout = millis();  // update arp_timeout to the current time
            ...
            arp_timeout = millis(); // update arp_timeout to the current time
            while(millis() - arp_timeout < 15) {
              network.update();
            }
            network.multicast(header, &msgTx->message, msgTx->size, 1); // Send to Level 1
          } else { // All other nodes (aside from the master node)
            ok = network.write(header, &msgTx->message, msgTx->size);					
          }
        }
      }
    } else {
      // TUN always needs to use RF24Mesh for address assignment AND resolution
